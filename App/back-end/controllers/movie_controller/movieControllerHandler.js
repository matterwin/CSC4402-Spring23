...
const postMovieHandler = (req, res) => {
    ...
    const endFilePath = `${splitFilePath[splitFilePath.length - 2]}/${splitFilePath[splitFilePath.length - 1]}/${file.originalname}`;

    console.log(body);

    executeQuery('sql/movie/postMovie.sql', [body.name,
                                             body.description,
                                             body.length,
                                             body.releaseDate,
                                             body.genre,
...
const getAllMovieHandler= (req, res) => {
    executeQuery('sql/movie/getAllMovie.sql', undefined, (queryResult) => {
...
const getMovieHandler = (req, res) => {
    ...
    executeQuery('sql/movie/getMovie.sql', [id], (queryResult) => {
        if(!queryResult.length){
            res.status(404).send();
            return;
        }
...
const deleteMovieHandler = (req, res) => {
    ...
    executeQuery('sql/movie/deleteMovie.sql', [id], (queryResult) => {
