...
const controllerRouterList = [
  ...
  require("./controllers/movie_review_controller/movieReviewController"),
];
...
controllerRouterList.forEach((routerObject) => {
  ...
  routerObject.routerRouteDetails.forEach((routerDetails) => {
    console.log(
      ` - ${routerDetails.type}: ${process.env.APP_URL}:${process.env.PORT}/${process.env.API_BASE_ROUTE}${routerDetails.path}`
    );
  });
